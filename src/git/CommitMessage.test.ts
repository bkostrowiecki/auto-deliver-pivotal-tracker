...
describe('Commit', () => {
    it('Should get right commit model - usual commit in system', () => {
        let commit = new CommitMessage('[#12345678] Commit message');
    ...
    it('Should get right commit model - commit with word in square brackets', () => {
        let finishesCommit = new CommitMessage('[Finishes #12345678] Commit message');

        expect(finishesCommit.getTaskHashes().length).toEqual(1);
        expect(finishesCommit.getTaskHashes()[0]).toEqual('12345678');

        let fixesCommit = new CommitMessage('[Fixes #12345678] Commit message');
    ...
    it('Should get right commit model - commit with word in square brackets', () => {
        let commit = new CommitMessage('[#12345678 #22345678] Commit message');
    ...
    it('Should get right commit model - commit with word in square brackets', () => {
        let commit = new CommitMessage('[Finishes #12345678][Finishes #22345678] Commit message');
    ...
    it('Should get empty array if there are no tasks in commit description', () => {
        let commit = new CommitMessage('Some commit without task hash');

        expect(commit.getTaskHashes().length).toEqual(0);
    });

    it('Test it', () => {
        let commit = new CommitMessage('[#150711864] Filtrowanie grup');
        
        expect(commit.getTaskHashes()[0]).toEqual('150711864');
    });

    it('Should ignore pull request hash', () => {
        let commit = new CommitMessage('Merge pull request #176 from Epicode/feature/zmiana-komunikatu-o-niewspieranych-typach-pytan-w-ankiecie');
    ...
    it('Should ignore pull request hash and get feature hash', () => {
        let commit = new CommitMessage('Merge pull request #176 from Epicode/feature/zmiana-komunikatu-o-niewspieranych-typach-pytan-w-ankiecie [Finishes #1234567]');
